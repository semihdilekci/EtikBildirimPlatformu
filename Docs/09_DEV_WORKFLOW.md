...
| `infra` | Docker, CI/CD, IaC, deployment |
...
- `.env.example`, `docker-compose*.yml` — ortam konfigürasyonu
...
| Ortam | Hosting | DB | Object Storage | Secret Kaynağı | Deploy Trigger |
|---|---|---|---|---|---|
| `dev` | Local (Docker Compose) | Local PostgreSQL container | Local MinIO container | `.env.local` dosyası | Manuel (`pnpm dev`) |
| `staging` | AWS ECS Fargate (staging hesabı) | RDS PostgreSQL (staging) | S3 (staging bucket) | AWS Secrets Manager (staging) | develop → staging PR merge |
| `production` | AWS ECS Fargate (prod hesabı, eu-central-1) | RDS PostgreSQL (prod, private subnet) | S3 (prod bucket, VPC Endpoint) | AWS Secrets Manager (prod) | staging → main PR merge + human approval |

### 4.2 Ortam İzolasyonu

prod, staging ve dev ortamları ayrı ağ, ayrı veritabanı, ayrı object storage bucket, ayrı KMS key scope ve ayrı service account kullanır. Non-prod ortamların production secret'larına, production object key'lerine veya production KMS key alias'larına erişimi olmaz. Development ve test ortamlarında gerçek etik bildirim verisi kullanılmaz; production verisi maskesiz kopyalanamaz.
...
| `AWS_REGION` | Evet | AWS region | `eu-central-1` |
| `AWS_KMS_KEY_ALIAS_FIELD` | Evet | Per-field encryption KEK alias | `alias/ethics-field-key` |
| `AWS_KMS_KEY_ALIAS_DOCUMENT` | Evet | Per-document encryption KEK alias | `alias/ethics-document-key` |
| `S3_BUCKET_DOCUMENTS` | Evet | Doküman storage bucket | `ethics-documents-prod` |
| `S3_BUCKET_QUARANTINE` | Evet | Quarantine bucket | `ethics-quarantine-prod` |
| `S3_ENDPOINT` | Hayır | S3-compatible endpoint (dev: MinIO) | `http://localhost:9000` |
...
| `CLAMAV_HOST` | Evet | ClamAV tarama servisi | `clamav:3310` |
...
#   - OIDC_CLIENT_ID ve OIDC_CLIENT_SECRET: team password manager'dan
#   - Diğer değerler .env.example'daki varsayılanlarla çalışır

# ─── Infrastructure ───
docker compose up -d
# Servisler:
#   postgres  → localhost:5432 (ethics_dev DB)
#   minio     → localhost:9000 (S3-compatible object storage)
#   clamav    → localhost:3310 (malware scanner)

# ─── Database ───
pnpm --filter api prisma:migrate:dev    # Migration'ları çalıştır
...
```yaml
# docker-compose.yml (referans yapı — tam dosya repo'da)
services:
  postgres:
    image: postgres:16-alpine
    environment:
      POSTGRES_DB: ethics_dev
      POSTGRES_USER: ethics
      POSTGRES_PASSWORD: ethics_dev_password
    ports: ["5432:5432"]

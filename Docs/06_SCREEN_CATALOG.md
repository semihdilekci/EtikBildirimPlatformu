...
| S-PENDING-REPORTS | `/app/reports/pending` | InternalLayout | council_secretary (`case:pre_review`) | Kritik |
...
📥 Bekleyen Bildirimler               → /app/reports/pending    [council_secretary — case:pre_review]
...
## S-PENDING-REPORTS — Bekleyen Bildirimler

**Route:** `/app/reports/pending`
**Erişim:** `council_secretary` — `PermissionGate(case:pre_review)`; diğer roller menüde görmez
**Layout:** InternalLayout
**Seviye:** Kritik

### Görsel Yapı

1. **Sayfa başlığı** — "Bekleyen Bildirimler".
2. **Filtre çubuğu** — Şirket dropdown, "Yalnızca acil risk" toggle. Filtreler URL query param'larına yansır.
3. **Bildirim tablosu (DataTable)** — Kolonlar: Takip Kodu (maskeli), Şirket, Kategori Grubu, Gönderim Tarihi, Acil Risk (badge). Satır tıklama → detay drawer/sayfa. Liste yanıtında `incidentDescription` gösterilmez.
4. **Detay önizleme (PendingReportDetailDrawer)** — `GET /reports/:id` ile metadata + maskeli özet. "Vaka Aç" butonu → `OpenCaseDialog` (Faz 5.1 İterasyon 4).
5. **Pagination** — Cursor-based (`nextCursor`).

### Veri Kaynağı

| Endpoint | Method | Amaç |
|---|---|---|
| `GET /api/v1/reports/pending` | GET | Bekleyen bildirim listesi |
| `GET /api/v1/reports/:id` | GET | İç önizleme (field-masked) |
| `POST /api/v1/cases` | POST | Bildirimden vaka açma (İterasyon 4 UI) |

### Durumlar

- **Empty:** "Bekleyen bildirim bulunmuyor." — CTA yok.
- **Error:** "Bildirimler yüklenemedi." + yeniden dene.
- **Loading:** Tablo skeleton.

### Güvenlik Notları

- Takip parolası ve decrypt alanları listede/önizlemede plaintext olarak gösterilmez.
- `admin` rolü bu ekrana erişemez.
- Clearance yetersiz bildirimler listede görünmez (backend scope).
...
2. **Filtre çubuğu (TaskFilters)** — Yatay filtre alanı. Filtreler: Durum (multi-select: Açık, Devam Eden, Beklemede, Tamamlanmış, İptal), **Tür** (dropdown: Vaka Görevi / Yapılandırma Onayı / Tümü), Görev Tipi (dropdown — yalnızca `kind=WORKFLOW`; task_type enum'ları), SLA Durumu (dropdown: Zamanında, Uyarı, Aşım — yalnızca workflow), İlişkili Vaka (text input — yalnızca workflow). Filtreler URL query param'larına yansır. "Filtreleri Temizle" linki.
...
4. **Görev tablosu (DataTable)** — TanStack Table + MUI. Kolonlar: **Tür** (Vaka Görevi / Yapılandırma Onayı badge), Görev/Onay Tipi (etiket + ikon), İlişkili Vaka (workflow'da link; onayda "—"), Durum, SLA Durumu (workflow; onayda "—"), Son Tarih / Talep Tarihi, Oluşturma Tarihi. Cursor-based pagination. Varsayılan sıralama: PENDING önce; workflow'da SLA aşımı önceliği, onaylarda `requestedAt` desc.
...
| Endpoint | Method | Amaç |
|---|---|---|
| `GET /api/v1/tasks` | GET | Birleşik workflow + onay listesi |

Query key'ler:
- `tasks.list(filters)` → staleTime: 30 sn, gcTime: 5 dk

**Onay satırları (`kind=APPROVAL`):** Checker rolüne sahip kullanıcılar görür; maker kendi talebini görür ama detayda decide butonu yok.

### State Yönetimi

- **Server state:** `tasks` — TanStack Query ile paginated fetch.
- **Local state:** Yok — filtre state'i URL'de.
- **URL state:** `status`, `kind`, `taskType`, `slaStatus`, `caseId`, `sortBy`, `sortOrder`, `cursor`.
- **Form state:** Yok.
...
- Görev tipi etiketi Türkçe gösterilir; backend `taskTypeLabel` veya `approvalCategoryLabel` alanı döndürür.
- **Yapılandırma onayları** vaka SLA'sına tabi değildir; `slaStatus` ve `dueAt` null.
- `council_secretary` checker rolündeyse admin kullanıcı ekranına erişmeden rol ataması onayını görev listesinden yapabilir.
...
**`kind=APPROVAL` dalı (Yapılandırma Onayı):**

1. **Üst bilgi** — Onay kategorisi etiketi + durum badge (SLA göstergesi yok).
2. **Onay bilgi kartı** — Maskeli özet, talep eden (display name), talep tarihi, hedef checker rolü.
3. **Aksiyon butonları** — `canDecide=true` iken: "Onayla" (primary), "Reddet" (error outlined). Maker kendi talebinde buton yok + bilgi metni.
4. **Karar dialog'u** — Onayla/Reddet → zorunlu gerekçe textarea → `POST /api/v1/tasks/:id/decide`.
5. Vaka linki, devir ve workflow tamamlama **gösterilmez**.

### Veri Kaynağı

| Endpoint | Method | Amaç |
|---|---|---|
| `GET /api/v1/tasks/:id` | GET | Workflow veya onay detayı |
| `POST /api/v1/tasks/:id/complete` | POST | Workflow görevini tamamla (`kind=WORKFLOW`) |
| `POST /api/v1/tasks/:id/delegate` | POST | Görev devri (`kind=WORKFLOW`) |
| `POST /api/v1/tasks/:id/decide` | POST | Onay/red (`kind=APPROVAL`) |
...
- **Checker birincil yolu:** `/app/tasks` birleşik listesi (`kind=APPROVAL`). Admin-only kullanıcı detayındaki "Bekleyen Onaylar" kartı maker/admin için tamamlayıcı kalır.
...
- Dropdown seçeneklerinde checker için maker rolü hariç tüm roller listelenir. Maker ve checker aynı rol seçildiğinde frontend uyarı gösterir: "Aynı rol hem talep eden hem onaylayan olamaz" — backend de bunu teknik olarak zorlar.
